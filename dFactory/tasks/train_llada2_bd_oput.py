        ...
        for _ in range(start_step, args.train.train_steps):
            ...
            for micro_batch in micro_batches:
                ...
                with torch.no_grad():
                    ...
                    orig_shape = semi_logits.shape[:-1]
                    vocab_size = semi_logits.shape[-1]
                    logits = semi_logits.reshape(-1, vocab_size)
                    token = semi_logits.argmax(dim=-1, keepdim=True)
                    semi_input_ids = token.view(*orig_shape)

                    active_block_mask = (micro_batch["input_ids"][:, :noisy_input_ids.shape[1]] == mask_token_id)

                    if micro_batch['flag'].item() is True:
                        micro_batch["input_ids"][:, :noisy_input_ids.shape[1]] = torch.where(
                            active_block_mask, 
                            semi_input_ids[:, :noisy_input_ids.shape[1]], 
                            micro_batch["input_ids"][:, :noisy_input_ids.shape[1]])
                model.train()
