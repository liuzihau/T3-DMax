...
class LLaDA2Model(nn.Module):
    ...
    def __init__(
        ...
        # DBet graph-safe feature tap (off by default; == the modeling_llada2_moe_sglang_dbet copy's only addition)
        self._dbet = None

    def enable_dbet_tap(self, sel_layers, max_bs=1, max_len=512, dtype=torch.bfloat16):
        """Turn on GRAPH-SAFE capture of the DBet drafter's features into FIXED preallocated buffers. Captures
        (hidden_states + residual) at eager hidden-state indices `sel_layers` (== the forward-hook tap's h_sel,
        since the split residual stream folds to h+r at a layer boundary) and the POST-final-norm hidden (h_last).
        The buffers are written by copy_ INSIDE forward() -> captured in the CUDA graph and refreshed on replay
        (Python hooks are not). Read with pop_dbet_features(bs, length) after each runner.forward()."""
        D = self.embed_dim
        n = self.config.num_hidden_layers
        sel = list(sel_layers)
        dev = self.norm.weight.device
        bufs = {k: torch.zeros(max_bs, max_len, D, dtype=dtype, device=dev) for k in sel + [n]}
        self._dbet = {"sel_at": {k - 1: k for k in sel}, "hlast_hs": n, "bufs": bufs, "sel": sel}

    def pop_dbet_features(self, bs, length):
        """(h_sel [bs,length,|sel|*D], h_last [bs,length,D]) from the most recent forward (cloned off the buffers)."""
        d = self._dbet
        h_sel = torch.cat([d["bufs"][k][:bs, :length] for k in d["sel"]], dim=-1).clone()
        h_last = d["bufs"][d["hlast_hs"]][:bs, :length].clone()
        return h_sel, h_last
