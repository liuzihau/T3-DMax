...
def decode_block_dbet(model, x, bs, be, attn, heavy_threshold, draft_threshold,
    ...
    while it < max_iters:
        ...
        if mloc.numel() > 0:
            ...
            # so the drafter's complete (bias-included) output IS its trained estimate of the heavy's next-step
            # distribution -- the coherent belief to hand the heavy. (An unbiased-re-feed variant was tried
            # 2026-07-13 on the "bias sharpens wrong chains" hypothesis; its pre-registered test failed:
            # 90.45 -> 89.46 on dt0.5_mixconf_k2_mk_force, within noise but no recovery -> reverted.)
